...
function draw() {
  ...
  if(gameState === "PLAY"){
    ...
    if(zombieFasterGroup.isTouching(bulletGroup)){
      ...
      if(score>150 || zombieGroup.isTouching(player) || zombieFasterGroup.isTouching(player) 
...
if(keyWentDown("space") || touches.length>0){
 shoot.play();
 bullet = createSprite(width/2-560,height/2-35,25,10);
...
function spawnZombieFaster(){
  ...
  zombieFaster.velocityX = -(3+3*score/10);
...
function spawnZombieGiant(){
  ...
  zombieGiant.scale = 3
  zombieGiant.velocityX = -(3+3*score/10);
